...
import { highest, medium, low } from './options.js'; // Note the .js extension


async function convertImageToSvg(inputPath, outputPath, size) {
    try {
      ...
      if (size < 100000){
       ...
       const svgString = imagetracerjs.imagedataToSVG(imageData, size < 100000 ? highest : size < 800000 ? medium : low);
  ...
  export const onUserDocumentUpdate = onDocumentUpdated("adm_products/{documentId}", async (change) => {
    ...
    if (oldStatus === 1 && newStatus === 2) {
      try {
        ...
        await convertImageToSvg(tmpPngFilePath, tmpSvgFilePath, fileStats.size);
